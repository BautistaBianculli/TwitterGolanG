package routers

import (
	"encoding/json"
	"net/http"
	"net/https"
	"strconv"

	"github.com/BautistaBianculli/TwitterGolanG/bd"
)

/*LeoTweetsSeguidores lee los tweets de todos nuestros seguidores*/
func LeoTweetsSeguidores(w http.ResponseWriter, r * http.Request){
	if len(r.URL.Query().Get("pagina")) < 1 {
		http.Error(w, "Porfavor enviar el parametro pagina", http.StatusBadRequest)
		return
	}

	page , err := strconv.Atoi(r.URL.Query().Get("pagina"))
	if err != nil{
		http.Error(w, "Enviar el parametro pagina como entero > 0", http.StatusBadRequest)
		return
	}

	respuesta, correcto := bd.LeoTweetsSeguidores(IDUsuario, page)
	if !correcto {
		http.Error(w, "Error al leer tweets", http.StatusBadRequest)
		return
	}

	w.Header().Set("Content-Type","application/json")
	w.WriteHeader(http.StatusCreated)
	json.NewEncoder(w).Encode(respuesta)
}